...
const getCountryFromIP = async (ip) => {
    try {
        ...
        if (response.data.success && response.data.country_code) {
...
const detectLanguage = async (req, res) => {
    try {
        ...
        if (countryCode === "XX") countryCode = null;
        ...
        if (!countryCode) {
            ...
            countryCode = await getCountryFromIP(ip.trim());
        }

        countryCode = countryCode ? countryCode.toUpperCase() : null;
        const languageCode = countryCode ? (countryToLanguage[countryCode] || "en") : "en";
        const languageName = languageNames[languageCode] || "English";

        res.json({ success: true, countryCode, languageCode, languageName });
    } catch (error) {
        res.status(500).json({
            success: false, message: "Language detection failed",
            countryCode: null, languageCode: "en", languageName: "English",
        });
    }
};
