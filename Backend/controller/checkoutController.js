...
exports.confirmAndSaveOrder = async (req, res) => {
    ...
    try {
        ...
        const serverSubtotal = calcSubtotal(items);
        let detectedPaymentMethod = paymentIntent.payment_method?.type || 'card';
        if (detectedPaymentMethod === 'card') {
            const walletType = paymentIntent.payment_method?.card?.wallet?.type;
            if (walletType === 'apple_pay') detectedPaymentMethod = 'apple_pay';
            else if (walletType === 'google_pay') detectedPaymentMethod = 'google_pay';
        }
...
exports.placePaypalOrder = async (req, res) => {
    try {
        ...
        const newOrder = new Order({
            userId: linkedUser ? linkedUser._id : null,
            customerInfo, items: mapItems(items), subtotal: serverSubtotal, finalTotal: serverSubtotal,
            note: note || customerInfo?.note || '', paymentMethod: 'paypal', paymentStatus: 'succeeded',
