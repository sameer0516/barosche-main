                  ...
                  <div className="form-group full">
                    ...
                    {(av.existingImages?.length > 0 || av.newImages?.length > 0) && (
                      <div className="img-preview-grid">
                        {(av.existingImages || []).map((src, i) => (<div key={`ex-${i}`} className="img-preview-item"><img src={`https://api.barosche.com${src}`} alt="" className="img-preview-thumb" width={100} height={100} /><button type="button" className="btn-remove-img" onClick={() => removeExistingImg(activeIdx, i)}>✕</button></div>))}
                        {(av.newImages || []).map((f, i) => (<div key={`nw-${i}`} className="img-preview-item"><img src={URL.createObjectURL(f)} alt="" className="img-preview-thumb" width={100} height={100} /><button type="button" className="btn-remove-img" onClick={() => removeNewImg(activeIdx, i)}>✕</button></div>))}
                      </div>
                    )}
                  </div>
