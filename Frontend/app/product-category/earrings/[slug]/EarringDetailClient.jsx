...
async function translateDynamicTexts(texts, targetLanguage) {
    ...
    try {
        ...
        if (!data.success) throw new Error("Dynamic translation failed");
        return data.translations;
    } catch (err) {
        console.error("EarringDetail dynamic content translation error:", err.message);
        return texts;
    }
}
...
function useTranslationAndCurrency() {
    ...
    useEffect(() => {
        ...
        async function run() {
            try {
                ...
                const { languageCode: lang, countryCode } = detectData;

                if (!cancelled) setLanguageCode(lang || "en");
    ...
    return { strings, currency, status, languageCode };
}
                ...
                {options.map((opt) => {
                    ...
                    return (
                        <button
                            ...
                            style={{ ...getMetalStyle(opt.value, isActive), transition: 'all 0.25s ease', cursor: 'pointer' }}
        ...
        <div className="jd-delivery-section">
            <div className="jd-delivery-item">
                ...
                <div className="jd-delivery-right">
                    ...
                    <span className="jd-delivery-variant">{T.expressVariants}</span>
                </div>
            </div>
            <div className="jd-delivery-item">
                <div className="jd-delivery-icon">
...
export default function EarringDetailClient({ slug }) {
    ...
    useEffect(() => {
        ...
        async function run() {
            const av = product.variants?.[selectedVariantIndex] || getFirstVariant(product);
            const rawTitle = av.title || product.title || product.name || "";
            const rawDescription = av.description || product.description || "";
            const rawMaterials = cleanList(av.materials?.length > 0 ? av.materials : (product.materials || []));
            const rawGemstones = cleanList(av.gemstones?.length > 0 ? av.gemstones : (product.gemstones || []));

            const textsToTranslate = [rawTitle, rawDescription, ...rawMaterials, ...rawGemstones];
            const translations = await translateDynamicTexts(textsToTranslate, languageCode);

            if (cancelled) return;

            let idx = 0;
            const tTitle = translations[idx++] ?? rawTitle;
            const tDescription = translations[idx++] ?? rawDescription;
            const tMaterials = rawMaterials.map(() => translations[idx++]);
            const tGemstones = rawGemstones.map(() => translations[idx++]);

            setTranslatedProduct({ title: tTitle, description: tDescription, materials: tMaterials, gemstones: tGemstones });
        }

        run();
        return () => { cancelled = true; };
    }, [product, selectedVariantIndex, languageCode]);
    useEffect(() => {
        setSelectedImageIndex(0);
    ...
    }, [product, selectedVariantIndex, selectedMetal]);
    ...
    const handleAddToCart = useCallback(() => {
        ...
        const cartItem = {
            ...
            images: varImgs,
            img: varImgs[0] || '',
            metal: selectedMetal !== ALL_METAL ? selectedMetal : null,
            qty,
            slug: product.slug,
            category: product.category,
        };
    ...
    }, [product, selectedVariantIndex, qty, selectedMetal]);
    ...
    const videos = activeVariant.videos?.length > 0
    ...
    const displayMaterials = translatedProduct?.materials?.length > 0
        ...
        : cleanList(activeVariant.materials?.length > 0 ? activeVariant.materials : (product.materials || []));
    const displayGemstones = translatedProduct?.gemstones?.length > 0
        ? translatedProduct.gemstones
        : cleanList(activeVariant.gemstones?.length > 0 ? activeVariant.gemstones : (product.gemstones || []));
                                    ...
                                    {mediaRows.map((row, rowIdx) => (
                                        <div key={`${selectedVariantIndex}-row-${rowIdx}`} className="jd-img-row">
                                            {row.map((item, colIdx) => {
                                                ...
                                                return (
                                                    <div key={`${selectedVariantIndex}-media-${globalIdx}`}
