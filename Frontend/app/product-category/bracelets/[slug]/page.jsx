...
const CATEGORY_NAME = "Bracelets";
const CATEGORY_SLUG = "bracelets";
...
async function getRelatedProducts(category, slug) {
    try {
        ...
        return (data.products || []).filter(p => p.slug !== slug).slice(0, 5);
    } catch {
        return [];
    }
}
...
export async function generateStaticParams() {
    return generateCategoryStaticParams(CATEGORY_NAME);
}
...
export async function generateMetadata({ params }) {
    ...
    const title = `${product.title || product.name} | Barosche`;
    const description = (product.description || '').slice(0, 160);

    return {
        title,
        description,
        keywords: [product.category, 'bracelets', 'barosche', 'fine jewellery', 'statement bracelets', 'handcrafted jewellery']
            .filter(Boolean).join(', '),
...
export default async function BraceletsDetailPage({ params }) {
    ...
    const relatedProducts = product
        ? await getRelatedProducts(product.category || CATEGORY_NAME, slug)
        : [];
