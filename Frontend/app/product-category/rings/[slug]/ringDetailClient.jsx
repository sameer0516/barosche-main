...
// 🆕 sizeError prop add kiya — agar size required hai aur select nahi kiya to red border + error text dikhega
...
//  MAIN COMPONENT
// ─────────────────────────────────────────────────────────
export default function RingDetailClient({ slug }) {
    ...
    useEffect(() => {
        setSelectedImageIndex(0);
        setSelectedSize(null);
        setSizeError(false); // 🆕 variant change hone pe error bhi reset karo
    ...
    }, [product, selectedVariantIndex, selectedMetal, selectedColor, selectedSize, stoneColors, T.all]);
    ...
    // 🆕 size select hote hi error clear kar do
    ...
    const handleAddToCart = useCallback(() => {
        ...
        // 🆕 Agar is variant ke paas sizes hain aur user ne select nahi kiya, to add-to-cart block karo
        const sizesAvailable = activeVariant.sizes && activeVariant.sizes.length > 0;
        if (sizesAvailable && !selectedSize) {
            setSizeError(true);
            // size selector tak smoothly scroll kar do taaki user ko dikhe
    ...
    }, [product, selectedVariantIndex, qty, selectedColor, selectedSize, selectedMetal, stoneColors, T.all]);
    ...
    // Accordion content — translated strings use ho rahi hain
                ...
                <>
                    ...
                    <div className="jd-desc-meta">
                        {displayMaterials.length > 0 ? (
                            ...
                            <p><strong>{T.attrMaterial}:</strong> {product.material}</p>
                        ) : null}
    ...
    return (
        <>
            <div className="jd-page">
                ...
                {/* Translation loading bar — Rings.js pattern */}
                {tStatus === "loading" && (
