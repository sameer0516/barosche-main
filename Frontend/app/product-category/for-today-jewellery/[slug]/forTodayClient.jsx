    ...
    useEffect(() => {
        ...
        async function run() {
            try {
                ...
                console.error("ForTodayClient translation error:", err.message);
    ...
    return (
        <div className="jd-attr-group">
            ...
            <div className="jd-attr-pills">
                {items.map((item) => (
                    <span key={item} className="jd-attr-pill" style={pillStyle}>{item}</span>
                ))}
            </div>
        </div>
    );
...
function MetalTypeSelector({ metalTypes, selectedMetal, onSelect, T }) {
    ...
    const getMetalStyle = (metal, isActive) => {
        if (!isActive) {
            return { background: 'transparent', color: 'var(--lux-black, #1a1a1a)', border: '1.5px solid #d4cfc8' };
        }
        switch (metal) {
            case 'Gold':
            case 'Silver':
                return {
                    ...
                    color: '#fff', border: '1.5px solid #b8873a',
                    boxShadow: '0 2px 8px rgba(201,169,110,0.45)',
                };
            default:
                return {
                    background: 'linear-gradient(135deg, #2c2c2c 0%, #4a4a4a 100%)',
                    color: '#fff', border: '1.5px solid #1a1a1a',
...
//  MAIN COMPONENT
// ─────────────────────────────────────────────────────────
export default function ForTodayClient({ slug }) {
    ...
    }, [product, selectedVariantIndex, qty, selectedColor, stoneColors, selectedMetal, T.all]);
                ...
                <>
                    ...
                    <div className="jd-desc-meta">
                        ...
                        {displayGemstones.length > 0 ? (
                            ...
                            <p><strong>{T.attrGemstone}:</strong> {stoneColors[selectedColor]?.name}</p>
                        )}
                    </div>
                </>
            )
        },
        {
            ...
            content: (
                <ul className="jd-bullet-list">
                    <li>{T.ship1}</li><li>{T.ship2}</li>
                    <li>{T.ship3}</li><li>{T.ship4}</li><li>{T.ship5}</li>
                </ul>
            )
        },
        {
            ...
            content: (
                <ul className="jd-bullet-list">
                    <li>{T.care1}</li><li>{T.care2}</li>
                    <li>{T.care3}</li><li>{T.care4}</li>
                </ul>
            )
        }
    ];
                ...
                <div className={`jd-max-container${isMobile ? ' jd-mobile-page' : ''}`}>
                    ...
                    {isMobile ? (
                        <div className="jd-mobile-layout">
                            <MobileSlider
                                images={images} getImgSrc={getImgSrc}
                            ...
                            <div className="jd-mobile-info">
                                ...
                                {allMetalTypes.length > 0 && (
                                    <MetalTypeSelector metalTypes={allMetalTypes} selectedMetal={selectedMetal}
                                        onSelect={handleMetalSelect} T={T} />
                                )}
                                ...
                                <div className="jd-mobile-qty-row">
                                    ...
                                    <button className="jd-btn-wishlist" aria-label={T.addToWishlist}>
                        ...
                        <div className="jd-related-grid">
                            {relatedProducts.map((rp) => (
                                <Link key={rp._id || rp.slug}
                                    href={`/product-category/for-today-jewellery/${rp.slug}`}
