// app/product-category/for-today-jewellery/[slug]/page.jsx

import ForTodayClient from './forTodayClient.jsx';
...
const CATEGORY_NAME = "For Today";
const CATEGORY_SLUG = "for-today-jewellery";
...
export async function generateStaticParams() {
    return generateCategoryStaticParams('For Today');
}
...
export async function generateMetadata({ params }) {
    ...
    const pageUrl = `${siteUrl}/product-category/for-today-jewellery/${product.slug}`;
    ...
    const title = `${product.title || product.name} | Barosche For Today`;
    const description = (product.description || '').slice(0, 160);

    return {
        title,
        description,
        keywords: [product.category, 'for today', 'barosche', 'fine jewellery', 'everyday jewellery']
            .filter(Boolean).join(', '),
...
export default async function ForTodayPage({ params }) {
    const { slug } = await params;
    const product = await getProduct(slug);
