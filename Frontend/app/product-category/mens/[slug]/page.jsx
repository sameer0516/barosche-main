// app/product-category/mens/[slug]/page.jsx
...
const CATEGORY_NAME = "Mens";
const CATEGORY_SLUG = "mens";
...
export async function generateStaticParams() {
    return generateCategoryStaticParams('Mens');
}
...
export async function generateMetadata({ params }) {
    ...
    // ✅ FIX: URL uses correct folder name (mens, not rings)
    const pageUrl = `${siteUrl}/product-category/mens/${product.slug}`;
    ...
    const title = `${product.title || product.name} | Barosche Mens Jewellery`;
    const description = (product.description || '').slice(0, 160);

    return {
        title,
        description,
        keywords: [product.category, 'mens jewellery', 'barosche', 'fine jewellery', 'handcrafted jewellery']
            .filter(Boolean).join(', '),
...
export default async function MensDetailPage({ params }) {
    const { slug } = await params;
    const product = await getProduct(slug);
