// app/product-category/rings/[slug]/page.jsx  lowercase rings folder
...
export async function generateStaticParams() {
    try {
        const res = await fetch(
            `${API_BASE}/api/products?category=Chosen Jewellery`,
            { next: { revalidate: 60 } }
        );
        if (!res.ok) return [{ slug: 'placeholder' }]; // fallback
        const data = await res.json();
        if (!data.success || !data.products?.length) return [{ slug: 'placeholder' }]; // fallback
        return data.products
            .filter((p) => p.slug)
            .map((p) => ({ slug: p.slug }));
    } catch {
        return [{ slug: 'placeholder' }]; // fallback
    }
}
